...
function Menubar({ onOpenSettings: r }) {
  const [n, o] = React.useState(null), i = { File: [{ label: "\u65B0\u898F", kbd: "Ctrl+N", action: () => window.__app?.newTab() }, { label: "\u958B\u304F", kbd: "Ctrl+O", action: () => window.__app?.openFile() }, null, { label: "\u4FDD\u5B58", kbd: "Ctrl+S", action: () => window.__app?.saveFile() }, { label: "\u5225\u540D\u3067\u4FDD\u5B58", kbd: "Ctrl+Shift+S", action: () => window.__app?.saveFileAs() }, null, { label: "\u9589\u3058\u308B", kbd: "Ctrl+W", action: () => window.__app?.closeActiveTab() }], Edit: [{ label: "\u5143\u306B\u623B\u3059", kbd: "Ctrl+Z", action: null }, { label: "\u3084\u308A\u76F4\u3057", kbd: "Ctrl+Y", action: null }], View: [{ label: "\u30A8\u30C7\u30A3\u30BF\u306E\u307F", kbd: "Ctrl+1", action: () => window.__app?.setViewMode("editor") }, { label: "\u30D7\u30EC\u30D3\u30E5\u30FC\u306E\u307F", kbd: "Ctrl+2", action: () => window.__app?.setViewMode("preview") }, { label: "\u4E21\u65B9\u8868\u793A", kbd: "Ctrl+3", action: () => window.__app?.setViewMode("both") }, null, { label: "\u76EE\u6B21\u3092\u8868\u793A/\u975E\u8868\u793A", kbd: "", action: () => window.__app?.toggleToc() }, { label: "\u30C6\u30FC\u30DE\u5207\u66FF", kbd: "", action: () => window.__app?.toggleTheme() }], Help: [{ label: "\u30AD\u30FC\u30DC\u30FC\u30C9\u30B7\u30E7\u30FC\u30C8\u30AB\u30C3\u30C8", kbd: "", action: null }, null, { label: "\u30D0\u30FC\u30B8\u30E7\u30F3\u60C5\u5831", kbd: "", action: () => window.__app?.showAbout() }] }, s = () => o(null);
  return useEffect(() => {
    if (n) {
      const l = () => s();
...
// Build metadata: replaced at build time by esbuild --define. Defaults to
// "dev" so unminified runs still produce a sensible value.
const __MDMD_BUILD = typeof __MDMD_BUILD_HASH__ !== "undefined"
  ? { hash: __MDMD_BUILD_HASH__, date: __MDMD_BUILD_DATE__, ver: __MDMD_BUILD_VER__ }
  : { hash: "dev", date: "unknown", ver: "dev" };
window.__MDMD_BUILD = __MDMD_BUILD;
function Statusbar({ tab: r, cursor: n, stats: o, dark: i }) {
  const buildTip = `MdMd ${__MDMD_BUILD.ver}\ncommit ${__MDMD_BUILD.hash}\nbuilt ${__MDMD_BUILD.date}`;
  return React.createElement("div", { className: "statusbar" }, React.createElement("span", { className: "sb-item" }, React.createElement("span", { className: "sb-dot", style: { opacity: r?.dirty ? 1 : 0.5 } }), r?.dirty ? "\u672A\u4FDD\u5B58\u306E\u5909\u66F4" : "\u4FDD\u5B58\u6E08\u307F"), React.createElement("span", { className: "sb-item", style: { opacity: 0.7 } }, r?.path || "\u2014"), React.createElement("span", { style: { flex: 1 } }), React.createElement("span", { className: "sb-item" }, "\u884C ", n.line, ", \u5217 ", n.col), React.createElement("span", { className: "sb-item" }, o.lines, " \u884C"), React.createElement("span", { className: "sb-item" }, o.words, " \u8A9E"), React.createElement("span", { className: "sb-item" }, o.chars, " \u6587\u5B57"), React.createElement("span", { className: "sb-item" }, "UTF-8"), React.createElement("span", { className: "sb-item" }, "LF"), React.createElement("span", { className: "sb-item" }, "Markdown"), React.createElement("span", { className: "sb-item" }, i ? "Dark" : "Light"), React.createElement("span", { className: "sb-item sb-build", title: buildTip, onClick: () => window.__app?.showAbout() }, __MDMD_BUILD.ver, " \xB7 ", __MDMD_BUILD.hash));
}
...
`);
  ...
  return useEffect(() => {
    ...
    window.__app = { newTab: E, openFile: _, saveFile: D, saveFileAs: A, closeActiveTab: P, toggleToc: () => l((e) => !e), toggleTheme: () => u((e) => !e), setViewMode: f, showAbout: () => alert(`MdMd\n\nバージョン: ${__MDMD_BUILD.ver}\nコミット: ${__MDMD_BUILD.hash}\nビルド日時: ${__MDMD_BUILD.date}\n\nhttps://github.com/trie0000/MdMd`) };
  }, [E, _, D, A, P]), useEffect(() => {
    const e = (t) => {
